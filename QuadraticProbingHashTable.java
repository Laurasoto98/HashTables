...
public class QuadraticProbingHashTable {
	...
	// Hint: first check the load factor after add - if the size is beyond rehash
	// first!
	public void insert(int x) {
		
		int size = HashTable.length;
		size--;
		double num = (double) currentSize / size;
		if (num > 0.75) {
		...
		if (HashTable[index] == null || !HashTable[index].isActive) {
			...
			if (find(x) == -1) {
				...
				do {
					...
					index = (j + (int) Math.pow(i, 2)) % HashTable.length;
					i++;
					if (HashTable[index] != null && !HashTable[index].isActive) {
						break;
					}

				} while (HashTable[index] != null && i < HashTable.length);
	...
	public void rehash() {
		HashEntry[] copyHash = HashTable;
		HashEntry[] newHash = new HashEntry[HashTable.length * 2];
		HashTable = newHash;
		for (int i = 0; i < copyHash.length; i++) {
			if (copyHash[i] != null) {
				insert(copyHash[i].element);
			}
		}
	...
	public void remove(int x) {
		...
		if (find(x) != -1) {
			HashTable[find(x)].isActive = false;
			currentSize--;
		}
